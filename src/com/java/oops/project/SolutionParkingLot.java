...
//    }
	...
	/**
	 * ParkingResult status 201 for success, 404 for failure vehicleType = 2 or 4
	 * for 2-wheeler or 4-wheeler vehicle
	 */
	public ParkingResult park(int vehicleType, String vehicleNumber, String ticketId) {
		if (!vehicleTypes.contains(vehicleType) || (vehicleNumber.isEmpty() && ticketId.isEmpty()))
	...
	/**
	 * - 201 success, 404 : vehicle not found or any other error, - exactly one of
	 * spotId, vehicleNumber or ticketId will be non empty
	 */
	...
	/**
	 * status = 200 : success, 404 : not found exactly one of spotId, vehicleNumber
	 * or ticketId will be non-empty
	 */
	...
	/*
	 * Converts a 2D array of String into a class based actual parking spot parking
	 * = [[ ["4-1","4-1","2-1","2-0"], ["2-1","4-1","2-1","2-1"],
	 * ["4-0","2-1","4-0","2-1"], ["4-1","4-1","4-1","2-1"]]]
	 */
	private ParkingSpot[][] getParkingSpots(int floor, String rows[][]) {
		...
		for (int row = 0; row < rows.length; row++) {
			for (int col = 0; col < rows[row].length; col++) {
				...
				if (spot.equals("2-1")) {
					spots[row][col] = new ParkingSpot(helper.getSpotId(floor, row, col), 2);
				} else if (spot.equals("4-1")) {
					spots[row][col] = new ParkingSpot(helper.getSpotId(floor, row, col), 4);
				}
			}
		}
		return spots;
...
class ParkingFloor {
	...
	public int removeVehicle(int row, int col) {
		if (row < 0 || row >= allSpots.length || col < 0 || col >= allSpots[0].length)
			return 404;
		ParkingSpot spot = allSpots[row][col];
		if (spot.getVehicleNumber().isEmpty() && spot.getTicketId().isEmpty())
			return 404;
		spot.setTicketId("");
